...
class CloudpickleWrapper(object):
    ...
    def __getstate__(self):
        import cloudpickle
        return cloudpickle.dumps(self._f)

    def __setstate__(self, ob):
        import pickle
        self._f = pickle.loads(ob)
...
class ShareVecEnv(ABC):
    ...
    def __init__(self, num_envs, observation_space, cent_observation_space, action_space):
        self.num_envs = num_envs
        self.observation_space = observation_space
        self.cent_observation_space = cent_observation_space
...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            remote.send((env.observation_space, env.cent_observation_space, env.action_space))
        else:
            raise NotImplementedError
...
class GuardSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        observation_space, cent_observation_space, action_space = self.remotes[0].recv()
        ShareVecEnv.__init__(self, len(env_fns), observation_space,
                             cent_observation_space, action_space)
...
def shareworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            remote.send(
                (env.observation_space, env.cent_observation_space, env.action_space))
...
class ShareSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        for remote in self.work_remotes:
            remote.close()
        self.remotes[0].send(('get_spaces', None))
        obs_space, cent_obs_space, act_space = self.remotes[0].recv()
        ShareVecEnv.__init__(self, len(env_fns), obs_space,
                             cent_obs_space, act_space)
    ...
    def step_wait(self):
        results = [remote.recv() for remote in self.remotes]
        self.waiting = False
        obs, cent_obs, rews, dones, infos, available_actions = zip(*results)
        return np.stack(obs), np.stack(cent_obs), np.stack(rews), np.stack(dones), infos, np.stack(available_actions)

    def reset(self):
        for remote in self.remotes:
            remote.send(('reset', None))
        results = [remote.recv() for remote in self.remotes]
        obs, cent_obs, available_actions = zip(*results)
        return np.stack(obs), np.stack(cent_obs), np.stack(available_actions)
...
class DummyVecEnv(ShareVecEnv):
    def __init__(self, env_fns):
        ...
        ShareVecEnv.__init__(self, len(
            env_fns), env.observation_space, env.cent_observation_space, env.action_space)
        self.actions = None
...
class ShareDummyVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        results = [env.step(a) for (a, env) in zip(self.actions, self.envs)]
        obs, cent_obs, rews, dones, infos, available_actions = map(
            np.array, zip(*results))

        for (i, done) in enumerate(dones):
            if 'bool' in done.__class__.__name__:
                if done:
                    obs[i], cent_obs[i], available_actions[i] = self.envs[i].reset()
            else:
                if np.all(done):
                    obs[i], cent_obs[i], available_actions[i] = self.envs[i].reset()
        self.actions = None

        return obs, cent_obs, rews, dones, infos, available_actions

    def reset(self):
        results = [env.reset() for env in self.envs]
        obs, cent_obs, available_actions = map(np.array, zip(*results))
